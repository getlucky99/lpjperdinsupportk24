...
async function apiCall(action, payload) {
  const url = APP.settings.scriptUrl;

  // Google Apps Script Web App tidak support JSON POST cross-origin dari file lokal.
  // Solusi: kirim sebagai form POST (application/x-www-form-urlencoded)
  // Apps Script menerima via e.parameter.data
  ...
  const res = await fetch(url, {
    method      : 'POST',
    body        : body,
    // Tidak set Content-Type header — biarkan browser set otomatis ke
    // application/x-www-form-urlencoded, menghindari CORS preflight
    redirect    : 'follow',
  });
  ...
  catch(e) { throw new Error('Response bukan JSON: ' + text.substring(0, 100)); }
...
function fillStep0() {
  ...
  document.getElementById('tgl-mulai').value         = l.tglMulai         || '';
  document.getElementById('tgl-selesai').value       = l.tglSelesai       || '';
...
function deleteUser(idx) {
  const u = APP.users[idx];
  if (!u) return;
  // IT Koordinator tidak bisa hapus Super Admin
...
async function testConnection() {
  ...
  try {
    ...
    status.className = 'connection-status error';
    status.innerHTML = '<i class="fas fa-circle-xmark"></i> Koneksi gagal: ' + e.message
      + '<br><small style="opacity:0.75;font-size:10px">Tips: Pastikan Apps Script di-deploy sebagai Web App,'
      + ' Execute as: Me, Who has access: Anyone</small>';
  }
}
...
function getDayCount(l) {
  ...
  const d1 = new Date(l.tglMulai), d2 = new Date(l.tglSelesai);
...
function generateId(){
  ...
  const num=String((APP.laporanList.length||0)+1).padStart(3,'0');
  return `LPJ-K24-ITSUPPORT-${year}-${num}`;
}
