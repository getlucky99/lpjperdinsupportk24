...
async function apiCall(action, payload, retries) {
  ...
  // AUTO-SWITCH: payload <= 1800 char → GET (cepat, no preflight)
  //              payload  > 1800 char → POST text/plain (payload besar, no preflight)
  // Threshold 1800 agar aman di semua browser (batas URL ~2000 char after encoding)
  const usePost = jsonPayload.length > 1800;
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      if (usePost) {
        ...
        // Apps Script doPost membaca via e.postData.contents
        ...
        // GET dengan ?data= untuk payload kecil
        const params = new URLSearchParams({ data: jsonPayload });
      ...
      try {
        ...
        if (text.includes('<html') || text.includes('<!DOCTYPE')) {
          throw new Error('Server return HTML — deploy ulang Apps Script sebagai New Version');
        }
...
// apiCallPost: untuk payload besar (upload file base64) — kirim via POST
// Apps Script doPost membaca e.postData.contents langsung sebagai JSON string
async function apiCallPost(action, payload) {
  const baseUrl = APP.settings.scriptUrl;
  if (!baseUrl) throw new Error('URL Apps Script belum dikonfigurasi');
  ...
  // Apps Script doPost menerima ini via e.postData.contents
...
async function loadFromServer() {
  ...
  try {
    ...
    // Load kategori perjalanan
    const kData = await apiCall('getKategori', {});
    if (kData.data && kData.data.length) APP.kategoriPerjalanan = kData.data;
    // Load kategori dinas
    try {
      const kdData = await apiCall('getKategoriDinas', {});
      if (kdData.data && kdData.data.length) APP.kategoriDinas = kdData.data;
    } catch(e) { console.warn('getKategoriDinas skip:', e.message); }
...
// Flag dan timer untuk mencegah request paralel ke server
...
async function saveLaporanToServer(laporan) {
  ...
  // Jika sedang menyimpan, tandai pending dan keluar
  // Request yang sedang berjalan akan kirim data terbaru setelah selesai
  if (_serverSaving) {
    _serverSavePending = true;
    return;
  }

  // Debounce server: tunggu 800ms setelah panggilan terakhir
  clearTimeout(_serverSaveTimer);
  _serverSaveTimer = setTimeout(async () => {
    _serverSaving = true;
    try {
      // Selalu ambil data terbaru dari APP.currentLaporan (bukan parameter lama)
      const current = APP.currentLaporan;
      await apiCall('saveLaporan', { laporan: current, username: APP.currentUser?.nama });
    } catch(e) {
      console.warn('saveLaporanToServer error:', e);
      showToast('Gagal simpan ke server: ' + e.message, 'warning');
    } finally {
      _serverSaving = false;
      // Jika ada request pending, jalankan sekali lagi dengan data terbaru
      if (_serverSavePending) {
        _serverSavePending = false;
        saveLaporanToServer(APP.currentLaporan);
      }
    }
  }, 800);
}
...
function startNewLaporan() {
  ...
  APP.currentLaporan = {
    ...
    noForm: '', tglPencairan: '', radItems: [], rpdItems: [], notaItems: [],
    totalRAD: 0, totalRPD: 0, totalNota: 0
  };
  // Buat folder Drive untuk laporan ini (async, tidak blokir UI)
  createLaporanFolder(newId);
...
function editLaporan(id) {
  ...
  // Reset filter RPD DULU sebelum set rpdRows (fix timing issue di HP)
  const _efp = document.getElementById('rpd-filter-pelaksana');
  const _eft = document.getElementById('rpd-filter-tujuan');
  if (_efp) { _efp.value = ''; _efp.innerHTML = '<option value="">— Semua Pelaksana —</option>'; }
  if (_eft) { _eft.value = ''; _eft.innerHTML = '<option value="">— Semua Tujuan —</option>'; }

  APP.rpdRows  = (l.rpdItems  || []).map(r => ({
    ...r,
...
function fillStep0() {
  ...
  // Set via JS property (bukan HTML attribute) — fix Safari/iOS input[type=date]
  // Delay 50ms lebih reliable di HP yang lambat (bukan 0ms)
  setTimeout(() => {
    const elMulai   = document.getElementById('tgl-mulai');
    const elSelesai = document.getElementById('tgl-selesai');
    if (elMulai)   elMulai.value   = normalizeDate(l.tglMulai)   || '';
    if (elSelesai) elSelesai.value = normalizeDate(l.tglSelesai) || '';
  }, 50);
  document.getElementById('keterangan-umum').value   = l.keteranganUmum   || '';
  document.getElementById('no-form').value            = l.noForm           || '';
...
function saveStep0() {
  ...
  Object.assign(APP.currentLaporan, {
    ...
    noForm        : (document.getElementById('no-form')?.value || '').trim(),
    tglMulai, tglSelesai,
  ...
  // Simpan ke list sebelum pindah step
  const _idx0 = APP.laporanList.findIndex(l => l.id === APP.currentLaporan.id);
  const _copy0 = JSON.parse(JSON.stringify(APP.currentLaporan));
  if (_idx0 >= 0) APP.laporanList[_idx0] = _copy0;
  else            APP.laporanList.push(_copy0);
  saveToStorage();
  try { updateStats(); }        catch(e){}
  try { renderRecentTable(); }  catch(e){}
  try { renderLaporanTable(); } catch(e){}
  saveLaporanToServer(APP.currentLaporan);
  goToStep(1);
  renderRADTable();
  // Set semua date input di Tahap 2 via JS property (fix Safari/iOS)
  setTimeout(() => {
    const pcEl = document.getElementById('tgl-pencairan');
    if (pcEl) pcEl.value = normalizeDate(APP.currentLaporan.tglPencairan) || '';
  }, 50); // 50ms — lebih reliable di HP lambat
}
...
function resetRPDFilter() {
  ...
  if (sp) sp.value = '';
  if (st) st.value = '';
  renderRPDTable(); // render ulang tanpa filter → semua baris tampil
  showToast('Filter direset, semua baris ditampilkan.', 'success');
}
...
function renderRPDTable() {
  ...
  // Hitung jumlah pelaksana dan tujuan dengan aman
  const jmlPelaks = (l.pelaksana||[]).filter(Boolean).length;
  // Pastikan tujuanList benar-benar array dengan isi
  const tujuanArr  = Array.isArray(l.tujuanList) && l.tujuanList.filter(Boolean).length > 0
    ? l.tujuanList.filter(Boolean)
    : (l.tujuan ? [l.tujuan] : []);
  const jmlTujuan  = tujuanArr.length;
  ...
  const wrapFP    = elFP ? elFP.closest('.filter-group') : null;
  const wrapFT    = elFT ? elFT.closest('.filter-group') : null;
  ...
  if (filterBar) {
    ...
    if (elFP && showFilterPelaksana) {
      ...
      (l.pelaksana||[]).filter(Boolean).forEach(p => {
  ...
  // ---- Filter aktif — hanya filter jika ada nilai yang dipilih ----
  // Jika filter tidak ditampilkan ATAU dropdown value kosong/null → tampilkan semua
  const filterPelaksana = (showFilterPelaksana && elFP && elFP.value && elFP.value !== '')
    ? elFP.value : '';
  const filterTujuan    = (showFilterTujuan    && elFT && elFT.value && elFT.value !== '')
    ? elFT.value : '';

  // ---- Filter rows — jika filter kosong ATAU rpdRows belum siap, SEMUA baris tampil ----
  // Safety: jika APP.rpdRows belum ada datanya, tampilkan semua tanpa filter
  const safeRows = APP.rpdRows || [];
  const visibleRows = (filterPelaksana || filterTujuan)
    ? safeRows.filter(row => {
        if (filterPelaksana && (row.pelaksana || '') !== filterPelaksana) return false;
        if (filterTujuan    && (row.tujuan    || '') !== filterTujuan)    return false;
        return true;
      })
    : safeRows; // Tidak ada filter aktif → tampilkan semua
  ...
  visibleRows.forEach((row) => {
    ...
    tr.innerHTML = `
      ...
      <td><input type="date" id="rpd-tgl-${i}" data-idx="${i}" onchange="updateRPDTanggal(${i},this.value)" style="width:140px"></td>
  ...
  // Set nilai tanggal via JavaScript property (bukan HTML attribute)
  // Delay 80ms — lebih reliable di HP lambat dibanding 0ms
  setTimeout(() => {
    (visibleRows || APP.rpdRows).forEach((row) => {
      const i   = APP.rpdRows.indexOf(row);
      const inp = document.getElementById(`rpd-tgl-${i}`);
      if (inp && row.tanggal) inp.value = normalizeDate(row.tanggal) || '';
    });
  }, 80);
...
// Update tanggal RPD — field string, tidak perlu parseFloat
function updateRPDTanggal(idx, val) {
  if (APP.rpdRows[idx] === undefined) return;
  // Normalisasi format tanggal agar selalu YYYY-MM-DD (aman di semua browser)
  const normalized = normalizeDate(val) || '';
  APP.rpdRows[idx].tanggal = normalized;
  // Sync ke currentLaporan langsung agar tidak hilang
  if (APP.currentLaporan.rpdItems && APP.currentLaporan.rpdItems[idx] !== undefined) {
    APP.currentLaporan.rpdItems[idx].tanggal = normalized;
  }
  debounceSave();
}
...
function applyNota(idx, file) {
  ...
  // Upload ke Google Drive jika live mode
  if (isLiveMode()) {
    uploadNotaToDrive(idx, file);
  }
}

async function uploadNotaToDrive(idx, file) {
  const laporanId = APP.currentLaporan?.id;
  if (!laporanId) return;
  // Tampilkan status upload
  const dropEl = document.getElementById(`nota-drop-${idx}`);
  if (dropEl) {
    dropEl.querySelector('.drop-label').innerHTML =
      `<i class="fas fa-spinner fa-spin" style="color:var(--k24-green)"></i> Mengupload...`;
  }
  try {
    // Baca file sebagai base64
    const base64 = await fileToBase64(file);
    // Pakai POST karena base64 bisa besar (1-5MB), tidak muat di URL GET
    const result = await apiCallPost('uploadNota', {
      laporanId,
      folderId  : APP.settings.driveId || '',
      fileName  : file.name,
      base64Data: base64,
      mimeType  : file.type || 'application/octet-stream',
    });
    if (result.success) {
      // Simpan fileId dari Drive ke row data
      APP.rpdRows[idx].notaFileId = result.fileId;
      APP.rpdRows[idx].notaUrl    = result.url;
      debounceSave();
      if (dropEl) {
        dropEl.querySelector('.drop-label').innerHTML =
          `<i class="fas fa-cloud-check" style="color:var(--k24-green)"></i> ${file.name} <small style="color:var(--k24-green);font-size:10px">✓ Tersimpan di Drive</small>`;
      }
      showToast(`Nota "${file.name}" berhasil diupload ke Drive.`, 'success');
    }
  } catch(e) {
    console.warn('Upload nota gagal:', e.message);
    if (dropEl) {
      dropEl.querySelector('.drop-label').innerHTML =
        `<i class="fas fa-triangle-exclamation" style="color:orange"></i> ${file.name} <small style="color:orange;font-size:10px">⚠ Gagal upload Drive</small>`;
    }
    showToast('Nota tersimpan lokal, gagal upload ke Drive: ' + e.message, 'warning');
  }
}
...
function saveStep2() {
  // Baca nilai tanggal dari DOM langsung untuk memastikan tidak ada yang terlewat
  // (kadang onchange belum trigger jika user tab-out cepat)
  // Baca tanggal dari DOM (fix Safari: value property, bukan attribute)
  document.querySelectorAll('#rpd-tbody input[type="date"]').forEach((input) => {
    const idx = parseInt(input.dataset.idx);
    if (!isNaN(idx) && APP.rpdRows[idx] !== undefined) {
...
function saveStep3(){
  // Sync notaItems dari DOM langsung untuk keandalan
  APP.notaRows.forEach((row, i) => {
    const qEl = document.getElementById(`nota-qty-${i}`);
    const sEl = document.getElementById(`nota-sat-${i}`);
    if (qEl) row.qty    = parseFloat(qEl.value) || 0;
    if (sEl) row.satuan = parseFloat(sEl.value) || 0;
    row.jumlah = (row.qty||0) * (row.satuan||0);
  });
  ...
  autoSaveLaporan();
  // Paksa simpan ke server sekarang (jangan tunggu debounce)
  saveLaporanToServer(APP.currentLaporan);
  goToStep(4); renderResume();
}
...
function finalizeLaporan(){
  ...
  // Pastikan laporan ada di list sebelum navigate
  const idx = APP.laporanList.findIndex(l => l.id === APP.currentLaporan.id);
  const copy = JSON.parse(JSON.stringify(APP.currentLaporan));
  if (idx >= 0) APP.laporanList[idx] = copy;
  else          APP.laporanList.push(copy);
  saveToStorage();
  saveLaporanToServer(APP.currentLaporan);
  // Refresh semua UI
  try { updateStats(); }        catch(e){}
  ...
  setTimeout(() => showPage('laporan-list'), 900);
}
...
function debounceSave() {
  ...
  _saveTimer = setTimeout(() => {
    ...
    if (APP.currentStep === 1) {
      ...
      // Deep copy termasuk field tanggal
      APP.currentLaporan.rpdItems  = APP.rpdRows.map(r => Object.assign({}, r));
      APP.currentLaporan.totalRPD  = APP.rpdRows.reduce((s,r)=>s+(r.subTotal||0),0);
    ...
    const idx = APP.laporanList.findIndex(l => l.id === APP.currentLaporan.id);
    const copy2 = JSON.parse(JSON.stringify(APP.currentLaporan));
    if (idx >= 0) APP.laporanList[idx] = copy2;
    else          APP.laporanList.push(copy2);
    saveToStorage();
    // Refresh stats & tabel di background (tidak mempengaruhi form yang sedang diisi)
    try { updateStats(); }        catch(e){}
    try { renderRecentTable(); }  catch(e){}
    try { renderLaporanTable(); } catch(e){}
    // saveLaporanToServer sudah punya debounce internal 800ms
    // dan cegah request paralel — aman dipanggil berkali-kali
    saveLaporanToServer(APP.currentLaporan);
    showSavedIndicator();
  }, 1200);
}
...
function autoSaveLaporan(){
  const l   = APP.currentLaporan;
  const idx = APP.laporanList.findIndex(x => x.id === l.id);
  const copy = JSON.parse(JSON.stringify(l));
  if (idx >= 0) APP.laporanList[idx] = copy;
  else          APP.laporanList.push(copy);
  saveToStorage();
  // Refresh dashboard & tabel agar laporan langsung tampil tanpa perlu logout
  try { updateStats(); }        catch(e){}
  try { renderRecentTable(); }  catch(e){}
  try { renderLaporanTable(); } catch(e){}
  saveLaporanToServer(l);
...
function goToStep(step){
  ...
  if(step===2) {
    // setTimeout agar currentLaporan & DOM sudah siap sepenuhnya
    // Fix: di HP lambat, data belum siap saat render langsung dipanggil
    setTimeout(() => {
      if (APP.rpdRows && APP.rpdRows.length) renderRPDTable();
    }, 50);
  }
...
function addKategori(){
  ...
  APP.kategoriDinas.push(val);
  saveToStorage(); updateKategoriSelects(); renderKategoriModal();
  if(isLiveMode()){
    apiCall('saveKategoriDinas',{kategoriList:APP.kategoriDinas})
      .then(()=>showToast('Kategori Dinas disimpan ke database.','success'))
      .catch(e=>showToast('Tersimpan lokal, gagal ke DB: '+e.message,'warning'));
  } else {
    showToast('Kategori Dinas ditambahkan.','success');
  }
  document.getElementById('new-kategori-input').value='';
}
function removeKategori(idx){
  APP.kategoriDinas.splice(idx,1);
  saveToStorage(); updateKategoriSelects(); renderKategoriModal();
  if(isLiveMode()){
    apiCall('saveKategoriDinas',{kategoriList:APP.kategoriDinas})
      .then(()=>showToast('Kategori Dinas dihapus dari database.','success'))
      .catch(e=>showToast('Terhapus lokal, gagal di DB: '+e.message,'warning'));
  }
}
...
function addKategoriPerjalanan(){
  ...
  APP.kategoriPerjalanan.push(val);
  saveToStorage();
  renderKategoriPerjalananModal(); renderRADTable(); renderRPDTable();
  if(isLiveMode()){
    apiCall('saveKategori',{kategoriList:APP.kategoriPerjalanan})
      .then(()=>showToast('Kategori disimpan ke database.','success'))
      .catch(e=>showToast('Tersimpan lokal, gagal ke DB: '+e.message,'warning'));
  } else {
    showToast('Kategori perjalanan ditambahkan.','success');
  }
  document.getElementById('new-kategori-perjalanan-input').value='';
}
function removeKategoriPerjalanan(idx){
  APP.kategoriPerjalanan.splice(idx,1);
  saveToStorage();
  renderKategoriPerjalananModal(); renderRADTable(); renderRPDTable();
  if(isLiveMode()){
    apiCall('saveKategori',{kategoriList:APP.kategoriPerjalanan})
      .then(()=>showToast('Kategori dihapus dari database.','success'))
      .catch(e=>showToast('Terhapus lokal, gagal di DB: '+e.message,'warning'));
  }
}
...
function generateDetailHtml(l) {
  ...
  // Jika tujuan > 1: tampilkan isi Keterangan (Tahap 1) bukan list tujuan
  // Jika tujuan = 1: tampilkan tujuan tunggal
  const multiTujuan3 = Array.isArray(l.tujuanList) && l.tujuanList.length > 1;
  const tujuanTxt = multiTujuan3
    ? (l.keteranganUmum || l.tujuanList.join(', ') || '-')
    : (l.tujuan || '-');
        ...
        <table style="width:100%;border-collapse:collapse;border:1.5px solid #333;margin-bottom:6px">
          <tr>
            ...
            <td style="width:160px;vertical-align:top;padding:0">
              ...
              <div style="padding:4px 6px;font-size:8px">No. Form :<br><span style="border-bottom:1px solid #999;display:block;min-height:12px;width:100%;margin-top:2px;font-family:monospace;font-weight:700">${l.noForm||''}</span></div>
            </td>
          </tr>
        </table>
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:6px">
          ...
          <div style="border-left:1px solid #ddd;padding-left:8px;display:flex;flex-direction:column;gap:10px">
            <div>
              ...
              <div style="display:flex;flex-direction:column;gap:3px;padding-left:2px">
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                  <span style="font-size:13px;font-weight:900;color:#000;line-height:1">☐</span>
                  <span style="font-size:8px;color:#000;font-weight:400">Budgeter</span>
                  <span style="border-bottom:1px solid #bbb;min-width:55px;font-size:7.5px;color:#000;padding-left:2px">(isi nomor Budget)</span>
                </div>
            ...
            <div>
              ...
              <div style="display:flex;flex-direction:column;gap:3px;padding-left:2px">
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                  <span style="font-size:11px;font-weight:900;color:#000;line-height:1">${isBeban?'●':'○'}</span>
                ...
                <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px">
                  <span style="font-size:11px;font-weight:900;color:#000;line-height:1">${isDitag?'●':'○'}</span>
                  <span style="font-size:8px;color:#000;font-weight:400">Ditagihkan</span>
                </div>
                <div style="display:flex;align-items:center;gap:6px">
                  <span style="font-size:11px;font-weight:900;color:#000;line-height:1">${isPotong?'●':'○'}</span>
        ...
        <table style="width:100%;border-collapse:collapse;border:1.5px solid #333;margin-bottom:6px;font-size:8px">
          <tr>
            ${[
              ...
              ['Diketahui oleh,', l.atasanLangsung||'',  'Atasan Langsung / Pimpinan Dept.'],
              ['Disetujui oleh,', l.pimpinanDept||'', 'Pimpinan Dept. (yang dibebankan*)', 'noparens'],
            ...
            <div style="display:grid;grid-template-columns:110px 6px 1fr;align-items:center;margin-top:2px">
              ...
              <div style="display:flex;gap:10px;align-items:center">
                <div style="display:flex;gap:4px;align-items:center">
                  <span style="font-size:13px;font-weight:900;color:#000;line-height:1">☐</span>
                  <span style="font-size:8px;color:#000;font-weight:700">TUNAI / CASH</span>
                </div>
                <div style="display:flex;gap:4px;align-items:center">
                  <span style="font-size:13px;font-weight:900;color:#000;line-height:1">☑</span>
                  <span style="font-size:8px;color:#000;font-weight:700">TRANSFER</span>
                </div>
              </div>
            </div>
          </div>
...
function generateTanpaNotaHtml(l) {
    ...
    <div class="lembar">
      ...
      <div class="header">
        ...
        <div class="qr-area">
          ...
          <div class="no-nota-box">
            <span class="no-nota-label">No. Nota&nbsp;:</span>
            <span class="no-nota-val">${l.noForm||''}</span>
          </div>
        </div>
      </div>
  ...
  const css = `<style>
    * { margin:0; padding:0; box-sizing:border-box; }
    ...
    .title-main { font-size:9.5pt; font-weight:900; color:#000; line-height:1.2; white-space:normal; word-break:break-word; }
    .title-sub  { font-size:6.5pt; color:#333; margin-top:2px; white-space:normal; }
    .qr-area    { width:52mm; min-width:52mm; max-width:52mm; display:flex; flex-direction:column; flex-shrink:0; overflow:hidden; }
    .qr-code-box { border-bottom:2.5px solid #000; padding:2mm 2mm; text-align:center; font-size:6.5pt; font-weight:bold; flex:1; display:flex; align-items:center; justify-content:center; }
    .no-nota-box { padding:2mm 2mm; font-size:7pt; display:flex; align-items:center; gap:3px; white-space:nowrap; overflow:hidden; box-sizing:border-box; width:100%; }
    .no-nota-label { font-weight:600; flex-shrink:0; font-size:7pt; }
    .no-nota-val { border-bottom:1.5px solid #000; flex:1; min-width:0; font-family:monospace; font-size:7pt; font-weight:700; padding:0 2px; overflow:hidden; text-overflow:ellipsis; display:block; max-width:100%; }
...
async function downloadNota() {
  ...
  if (!isLiveMode()) {
    ...
    alert(`Mode Demo — Nota yang terdaftar:\n\n${namaList}\n\nUntuk download ZIP, aktifkan koneksi ke Google Drive di menu Pengaturan.`);
    return;
  }

  showToast('Menyiapkan file ZIP nota dari Google Drive...', 'success');
  const btn = document.querySelector('[onclick="downloadNota()"]');
  if (btn) { btn.disabled = true; btn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Memproses...'; }

  try {
    const result = await apiCall('getZipUrl', {
      laporanId: l.id,
      folderId : APP.settings.driveId || '',
    });
    if (result.success && result.url) {
      // Buka URL download ZIP dari Drive
      window.open(result.url, '_blank');
      showToast('File ZIP sedang didownload.', 'success');
    } else {
      throw new Error(result.error || result.msg || 'Gagal membuat ZIP');
    }
  } catch(e) {
    showToast('Gagal download nota: ' + e.message, 'error');
  } finally {
    if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fas fa-cloud-arrow-down"></i> Download Nota'; }
  }
}
...
// Buat folder Drive untuk laporan baru (dipanggil setelah ID di-generate)
async function createLaporanFolder(laporanId) {
  if (!isLiveMode()) return; // hanya di mode live
  if (!APP.settings.driveId) {
    console.warn('Drive Folder ID belum dikonfigurasi di Pengaturan.');
    return;
  }
  try {
    await apiCall('createFolder', { laporanId, parentFolderId: APP.settings.driveId });
    console.log('Folder Drive dibuat:', laporanId);
  } catch(e) {
    console.warn('Gagal buat folder Drive:', e.message);
  }
}
