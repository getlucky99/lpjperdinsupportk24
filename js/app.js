...
function saveStep0() {
  ...
  saveLaporanToServer(APP.currentLaporan); // debounce server handle concurrency
  goToStep(1);
  renderRADTable();
...
function renderRPDTable() {
  ...
  visibleRows.forEach((row) => {
    ...
    tr.innerHTML = `
      ...
      <td><input type="date" value="${row.tanggal||''}" onchange="updateRPDTanggal(${i},this.value)" style="width:140px"></td>
...
function updateRPDTanggal(idx, val) {
  ...
  if (APP.currentLaporan.rpdItems && APP.currentLaporan.rpdItems[idx] !== undefined) {
    APP.currentLaporan.rpdItems[idx].tanggal = val || '';
  }
  debounceSave();
}
...
function saveStep2() {
  ...
  document.querySelectorAll('#rpd-tbody input[type="date"]').forEach((input, idx) => {
    if (APP.rpdRows[idx] !== undefined) {
      APP.rpdRows[idx].tanggal = input.value || '';
    }
  });
